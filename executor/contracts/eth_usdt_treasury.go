...
func (c *EthUsdtTreasuryContract) GetData(
	...
	jettonMasterAddress, err := result.Slice(0)
	if err != nil {
		return nil, err
	}
	...
	jettonWalletCode, err := result.Cell(1)
	if err != nil {
		return nil, err
	}

	oAppAddress, err := result.Slice(2)
	if err != nil {
		return nil, err
	}
	...
	dstEvmAddress, err := result.Int(3)
	if err != nil {
		return nil, err
	}

	ethEid, err := result.Int(4)
	if err != nil {
		return nil, err
	}

	maxBridgeAmount, err := result.Int(5)
	if err != nil {
		return nil, err
	}
...
func (c *EthUsdtTreasuryContract) GetBalance(ctx context.Context) (*big.Int, error) {
	...
	state, err := c.api.WaitForBlock(block.SeqNo).GetAccount(ctx, block, c.address)
	if err != nil {
